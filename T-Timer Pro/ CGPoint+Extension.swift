...
//  T-Timer Pro
//
//  Created by DellMac on 2019-12-05.
...
extension CGPoint {
    ...
    static func toTwoPI(value: CGFloat) -> CGFloat {
        return (value > 0) ? value : 2 * .pi + value
    }
    static func angleBetweenThreePoints(center: CGPoint, firstPoint: CGPoint, secondPoint: CGPoint, clockWise: Bool = true) -> CGFloat {
...
//            angleDiff *= -1
//        }
        let firstAngle = toTwoPI(value: atan2(firstPoint.y - center.y, firstPoint.x - center.x))
        let secondAnlge = toTwoPI(value: atan2(secondPoint.y - center.y, secondPoint.x - center.x))
        
        var angleDiff = (clockWise == false) ? firstAngle - secondAnlge : secondAnlge - firstAngle
    ...
    func angleBetweenPoints(firstPoint: CGPoint, secondPoint: CGPoint, clockWise: Bool = true) -> CGFloat {
        return CGPoint.angleBetweenThreePoints(center: self, firstPoint: firstPoint, secondPoint: secondPoint, clockWise: clockWise)
    }
